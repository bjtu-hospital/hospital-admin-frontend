<template>
  <div class="stats-card bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow cursor-pointer" 
       @click="handleClick">
    <div class="flex items-center justify-between">
      <div class="flex-1">
        <p class="text-gray-500 text-sm mb-1">{{ title }}</p>
        <h3 class="text-3xl font-bold text-gray-800">{{ formatNumber(value) }}</h3>
        <div v-if="subtitle" class="mt-2 text-sm text-gray-600">
          {{ subtitle }}
        </div>
      </div>
      <div class="icon-container ml-4" :class="`bg-${color}-100`">
        <div :class="`text-${color}-600 text-3xl`">
